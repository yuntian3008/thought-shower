---
description: Auto-chains /through-shower:start then /through-shower:ship in a single session. Use only for trivially small features where brainstorming, execution, and shipping all happen in one sitting. For multi-day features, use /start and /ship separately.
---

# /through-shower $ARGUMENTS

Single-command shortcut: run `/through-shower:start` and immediately `/through-shower:ship` in the same session.
